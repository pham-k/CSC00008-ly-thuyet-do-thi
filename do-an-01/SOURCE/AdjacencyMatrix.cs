...
namespace SOURCE
{
    public class AdjacencyMatrix {
        ...
        public void ReadData(string src = "./adjacency-matrix.txt")
        {
            ...
            List<int> startGoal = file
                .ReadLine()
                .Split(" ")
                .Select(s => int.Parse(s.Trim()))
                .ToList<int>();
            this.Start = startGoal[0];
            this.Goal = startGoal[1];
            for (int i = 0; i < v; i++) {
        ...
        public void GetPathMatrix() {
            ...
            this.PathMatrix = new int[v, v];
            // convert dictionary Graph to 2D array
            foreach (var item in this.Graph) {
                int i = item.Key;
                for (int j = 0; j < v; j++) {
                    if (item.Value[j] == 0) {
            ...
            for (int k = 0; k < v; k++) {
                for (int i = 0; i < v; i++) {
                    for (int j = 0; j < v; j++) {
                        // Console.WriteLine($"{this.PathMatrix[i, j]}, {this.PathMatrix[i, k]}, {this.PathMatrix[k, j]}");
                        this.PathMatrix[i, j] = this.PathMatrix[i, j] | (this.PathMatrix[i, k] & this.PathMatrix[k, j]);
                    }
                }
            }
        }
        public void PrintPathMatrix() {
            int v = GetNumberOfVertices();
            // Console.WriteLine(this.PathMatrix())
            for (int i = 0; i < v; i++) {
                for (int j = 0; j < v; j++) {
                    Console.Write($"{this.PathMatrix[i, j]} ");
                }
                Console.WriteLine();
            }
        }
        public void PrintConnectedType() {
            bool strongly = true;
            bool unilaterally = true;
            ...
            for (int i = 0; i < v; i++) {
                for (int j = 0; j < v; j++) {
                    if (j != i & this.PathMatrix[i, j] != this.PathMatrix[j, i]) {
                        disconnected = false;
                        break;
                    }
                }
